...
#----------------------------------------#
# Numerical example 3: Rugby ball 	 #
#----------------------------------------#

import numpy as np
import ksmma
from BP3import import *
...
max_ite = 50
gamma = 0.8 # allowed fluid volume

# Prescribe fluid around boundary

rho = gamma*np.ones(N)
edge = 0.05
C = 1/(3*dim) # distance from cell (triangle) midpoint to edge
for c in cells(mesh):
	p = c.midpoint()
	if (p.x()+C <= edge or p.x()-C >= 1-edge or \
p.y()+C <= edge or p.y()-C >= 1-edge):
		xmin[c.index()] = 1.0
		rho[c.index()] = 1.0
...
if 0:'''
#np.save('BP3rhoLS', xval)
xvalLS = np.load('BP3rhoLS.npy') 
...
ufile_pvd = File("results/BP3_velocity.pvd")
ufile_pvd << u
pfile_pvd = File("results/BP3_pressure.pvd")
pfile_pvd << p
rhofile_pvd = File("results/BP3_rho.pvd")
rhofile_pvd << rho

if useLS:
	wfile_pvd = File("results/BP3_vorticity.pvd")
	wfile_pvd << w

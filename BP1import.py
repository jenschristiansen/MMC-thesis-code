...
method = 'QNE1h' # methods=['MF','CY1','QNE2','QNE1h']
dim = 100
...
# Boundaries
def right_hole(x, on_boundary):
	return x[0] > (1.0 - DOLFIN_EPS) and (x[1] > 0.333 and x[1] < 0.666)
def right_wall(x, on_boundary):
	return x[0] > (1.0 - DOLFIN_EPS) and (x[1] <= 0.333 or x[1] >= 0.666)
def left(x, on_boundary): return x[0] < DOLFIN_EPS
def top_bottom(x, on_boundary):
	return x[1] > 1.0 - DOLFIN_EPS or x[1] < DOLFIN_EPS
def corner(x, on_boundary):
  	return x[0] > 1-DOLFIN_EPS and x[1] > 1-DOLFIN_EPS

# No-slip boundary condition for velocity
noslip = Constant((0.0, 0.0))
bc11 = DirichletBC(W.sub(0), noslip, top_bottom)
bc12 = DirichletBC(W.sub(0), noslip, right_wall)

g_bar_in = 1 # flow velocity at center of left boundary
g_bar_out = 3 # flow velocity at center of right boundary

# BCs for inflow / outflow
inflow = Expression(("1-(2*x[1]-1)*(2*x[1]-1)", "0.0"))
outflow = Expression(("3*(1-(6*x[1]-3)*(6*x[1]-3))", "0.0"))
bc21 = DirichletBC(W.sub(0), inflow, left)
bc22 = DirichletBC(W.sub(0), outflow, right_hole)

zero = Constant(0)
if usePrBC:
	bc31 = DirichletBC(W.sub(1), zero, right_hole)
else:
	bc31 = DirichletBC(W.sub(1), zero, corner, "pointwise")

# Collect boundary conditions
if usePrBC:
	bcs = [bc11, bc12, bc21, bc31] # pressure + velocity BCs
else:
	bcs = [bc11, bc12, bc21, bc22, bc31] # pin pressure, velocity BCs
	#bcs = [bc11, bc12, bc21, bc22] # plain velocity BCs
if useLS:
	bcs = [bc11, bc12, bc21, bc22, bc31] # pin pressure, velocity BCs

...
# Side BCs for velocity
upward = Constant((0.0, 1))
bc0 = DirichletBC(W.sub(0), upward, sides)

# Inflow/outflow BCs for velocity
inflow = upward
outflow = upward
bc11 = DirichletBC(W.sub(0), inflow, bottom)
bc12 = DirichletBC(W.sub(0), outflow, top)

# BC for pressure
zero = Constant(0)
bc21 = DirichletBC(W.sub(1), zero, top)
bc22 = DirichletBC(W.sub(1), zero, corner, 'pointwise')

# Collect boundary conditions
if usePrBC:
	bcs = [bc0, bc11, bc21] # pressure + velocity BCs
else:
	bcs = [bc0, bc11, bc12, bc22] # pin pressure, velocity BCs
	#bcs = [bc0, bc11, bc12] # plain velocity BCs
if useLS:
	bcs = [bc0, bc11, bc12, bc22] # pin pressure, velocity BCs

...
#----------------------------------------#
# Numerical example 2: Pipe bend 	 #
#----------------------------------------#

import numpy as np
import ksmma
from BP2import import *
...
max_ite = 50
gamma = 0.08*np.pi # allowed fluid volume
...
if 0:'''
#np.save('BP2rhoLS', xval)
xvalLS = np.load('BP2rhoLS.npy') 
...
ufile_pvd = File("results/BP2_velocity.pvd")
ufile_pvd << u
pfile_pvd = File("results/BP2_pressure.pvd")
pfile_pvd << p
rhofile_pvd = File("results/BP2_rho.pvd")
rhofile_pvd << rho

if useLS:
	wfile_pvd = File("results/BP2_vorticity.pvd")
	wfile_pvd << w
